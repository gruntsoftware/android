...
import ltd.grunt.brainwallet.presenter.activities.UpdatePinActivity;
import ltd.grunt.brainwallet.presenter.activities.intro.WriteDownActivity;
...
import static ltd.grunt.brainwallet.tools.manager.PromptManager.PromptItem.PAPER_KEY;
import static ltd.grunt.brainwallet.tools.manager.PromptManager.PromptItem.RECOMMEND_RESCAN;
import static ltd.grunt.brainwallet.tools.manager.PromptManager.PromptItem.SHARE_DATA;
import static ltd.grunt.brainwallet.tools.manager.PromptManager.PromptItem.UPGRADE_PIN;
...
public class PromptManager {
    ...
    public boolean shouldPrompt(Context app, PromptItem item) {
        ...
        switch (item) {
            case PAPER_KEY:
                return !ltd.grunt.brainwallet.tools.manager.BRSharedPrefs.getPhraseWroteDown(app);
            case UPGRADE_PIN:
                return BRKeyStore.getPinCode(app).length() != 6;
            case RECOMMEND_RESCAN:
                return ltd.grunt.brainwallet.tools.manager.BRSharedPrefs.getScanRecommended(app);
            case SHARE_DATA:
                return !ltd.grunt.brainwallet.tools.manager.BRSharedPrefs.getShareData(app) && !ltd.grunt.brainwallet.tools.manager.BRSharedPrefs.getShareDataDismissed(app);
                ...
                return new PromptInfo(app.getString(R.string.Prompts_RecommendRescan_title), app.getString(R.string.Prompts_RecommendRescan_body), new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        BRExecutor.getInstance().forLightWeightBackgroundTasks().execute(new Runnable() {
                            @Override
                            public void run() {
                                ltd.grunt.brainwallet.tools.manager.BRSharedPrefs.putStartHeight(app, 0);
