...
import static com.brainwallet.tools.animation.BRAnimator.animateBackgroundDim;
import static com.brainwallet.tools.animation.BRAnimator.animateSignalSlide;
...
//        shareTextMessage = (Button) rootView.findViewById(R.id.share_text);
    ...
    private void updateQr() {
        final Context ctx = getContext() == null ? BrainwalletApp.getBreadContext() : (Activity) getContext();
        BRExecutor.getInstance().forLightWeightBackgroundTasks().execute(new Runnable() {
            @Override
            public void run() {
